...
class Card extends Component {
    ...
    render() {
        ...
        const realizationPercent = (
            ((((price - buyPrice) * 100) / buyPrice) * 100) /
        ...
        return (
            <div className="card-box" key={key}>
                <div className="idea-date">
                    {this.formatDate(new Date(ideaDate))}
                </div>
                <div>
                    <img
                        className="logo inline-block"
                        src={'http://68.183.217.11/' + logoSrc}
                        alt="logo"
                    />
                    <div className="inline-block idea-name">{name}</div>
                </div>
                <div className="forecast inline-block">
                    Прогнозируемый доход:{' '}
                    <span className="forecast-percent">{forecast}%</span>{' '}
                    <span className="forecast-date"> за {period}</span>
                </div>
                <div>
                    <div className="buy-price inline-block">
                        <h3 className="price">${buyPrice}</h3>
                        <p>цена покупки</p>
                    </div>
                    <div className="sell-price inline-block">
                        <h3 className="price">${sellPrice}</h3>
                        <p>цена продажи</p>
                    </div>
                    <div className="price inline-block">
                        <h3 className="price">${price}</h3>
                        <p>текущая цена</p>
                    </div>
                    <div className="income inline-block">
                        <h3
                            className="income"
                            style={{ color: `${income < 0 ? 'red' : 'green'}` }}
                        >
                            {income}%
                        </h3>
                        <p>текущий доход</p>
                ...
                <div className="realization">
                    <p>
                        Идея реализована на <span> {realizationPercent}%</span>
                    </p>
                </div>
